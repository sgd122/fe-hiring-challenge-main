...
### Phase 12. 디자인 시각 정합성 보강 (Visual Fidelity Pass)

**목표:** 코드 acceptance / README 계약 검증만으로는 포착되지 않는 *시각
정합성* 결함을 디자인 참고 이미지(`docs/design.png`)와 실제 렌더링의
직접 비교로 보강.

**발견:** 디자인의 카드는 이미지 + 가격 정보 영역을 합친 wrapper
전체가 하나의 둥근 모서리이며, 이미지 좌우 하단은 직각으로 가격
정보 영역과 자연스럽게 맞붙는 형태. 그러나 `.content-card` /
`.skeleton-card`에 `border-radius`가 없어 카드 전체가 직각으로
렌더링되고 있었음. AI 정적 분석(Phase 1), architect 리뷰 (Phase 6),
Codex 적대적 리뷰(Phase 8/10), CCG 다중 모델 감사(Phase 11) 모두
통과한 뒤에도 남아 있던 시각 정합성 결함.

**적용 변경:** `src/components/Content.css`의 `.content-card`와
`.skeleton-card`에 `border-radius: 4px` 추가. 기존
`overflow: hidden`이 이미지 상단 두 모서리를 자연스럽게 클립하므로,
이미지 자체에는 radius를 주지 않아 좌우 하단을 직각으로 유지하고
가격 정보 영역이 카드 하단 둥근 모서리를 형성하도록 함. 첫 시도에서
`.card-image-wrapper` / `.skeleton-image`에도 radius를 줬으나 디자인은
이미지 하단을 직각으로 의도했음을 사람의 시각 비교로 재확인하고
제거 — 사람의 눈이 두 번 게이트로 작동한 사례. 회귀 없음 — 61
cases 그대로 그린 유지.

**시사점:** Acceptance criterion / 적대적 리뷰 / 다중 모델 감사가 모두
통과해도 *디자인 참고 이미지와의 시각 비교*는 별개 게이트. 본 워크플로
마지막 단계의 가장 단순한 검증은 결국 사람이 디자인을 띄우고 눈으로
대조하는 것이며, 그 게이트가 본 페이즈에서 한 건의 결함을 잡아냈다.
...
후 회귀 수정 → CCG 다중 모델 교차 감사 → 디자인 시각 정합성 보강까지
12단계 워크플로로 끌고 갔고, 각 단계마다 AI 출력을 그대로 받지 않고
검토/테스트/브라우저 검증을 거쳐 결정을 직접 내렸습니다. Phase 10에서는
Codex가 잡은 README 계약↔구현 미세 모순을 즉시 수정했고, Phase 11에서는
CCG가 제기한 두 건 중 한 건은 README 재독을 통해 거부했으며, Phase 12에서는
모든 자동 게이트를 통과한 뒤에도 남아 있던 카드 이미지 radius 결함을
사람의 시각 비교로 잡아 마무리 — AI 권고를 무비판 수용하지 않고,
사람의 마지막 게이트가 실제 가치를 만든 사례.
